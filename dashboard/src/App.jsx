...
function buildKpis(snaps, rich) {
  ...
  const histFlow = (field) => snaps.map(s => s[field]).filter(v => v != null && v > 0);
  ...
  return [
    { label: "Cycle Time",      sublabel: "In Progress → Done",  value: round1(last.cycleTime ?? 0),      unit: "d",  p85: last.cycleTimeP85    != null ? `${round1(last.cycleTimeP85)}d`    : null, delta: delta(last.cycleTime,    prev?.cycleTime,    true),  status: last.cycleTime    == null ? "neutral" : last.cycleTime    <= 3  ? "good" : last.cycleTime    <= 7  ? "warn" : "bad", history: histFlow("cycleTime"),    lowerBetter: true,  barMax: 14,  tooltip: "Median calendar days from 'In Progress' to 'Done'" },
    { label: "Time to Market",  sublabel: "Created → Done",      value: round1(last.timeToMarket ?? 0),   unit: "d",  p85: last.timeToMarketP85 != null ? `${round1(last.timeToMarketP85)}d` : null, delta: delta(last.timeToMarket, prev?.timeToMarket, true),  status: last.timeToMarket == null ? "neutral" : last.timeToMarket <= 7  ? "good" : last.timeToMarket <= 14 ? "warn" : "bad", history: histFlow("timeToMarket"), lowerBetter: true,  barMax: 28,  tooltip: "Median days from ticket creation to completion" },
    { label: "Flow Efficiency", sublabel: "Active / Total time", value: round1(last.flowEfficiency ?? 0), unit: "%",  p85: null,                                                                        delta: delta(last.flowEfficiency, prev?.flowEfficiency, false), status: last.flowEfficiency == null ? "neutral" : last.flowEfficiency >= 40 ? "good" : last.flowEfficiency >= 20 ? "warn" : "bad", history: histFlow("flowEfficiency"), lowerBetter: false, barMax: 100, tooltip: "% of total lead time the item was actively worked on" },
